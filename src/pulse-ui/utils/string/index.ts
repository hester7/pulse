 ...
 */
export const toConstantCase = (str: string) => str.replace(PATTERN1, "$1_$2").replace(PATTERN2, "$1_$2").replace(PATTERN4, "_").toUpperCase();
 ...
 */
export const toKebabCase = (str: string) => str.replace(PATTERN1, "$1-$2").replace(PATTERN2, "$1-$2").replace(PATTERN3, "-").toLowerCase();
...
export const toPascalCase = (str: string) =>
    str
        ...
        .replace(PATTERN7, " ")
        .replace(PATTERN8, (substr) => substr.charAt(0).toUpperCase() + substr.slice(1).toLowerCase())
        .replace(PATTERN9, "");
 ...
 */
export const toSnakeCase = (str: string) => str.replace(PATTERN1, "$1_$2").replace(PATTERN2, "$1_$2").replace(PATTERN4, "_").toLowerCase();
...
/**
 * Convert a string from `PascalCase` to normal case
 * @param {string} str
 * @returns {string}
 * @example
 * fromPascalToNormal('FooBarBaz') // → Foo Bar Baz
 * */
export const fromPascalToNormal = (str: string) =>
    str
        .replace(PATTERN10, " $1")
        .replace(PATTERN8, (substr) => substr.charAt(0).toUpperCase() + substr.slice(1).toLowerCase())
        .trim();
